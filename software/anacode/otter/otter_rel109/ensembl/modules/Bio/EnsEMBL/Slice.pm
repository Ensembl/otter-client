...
sub project {
  ...
  my $cs_name = shift;
  my $cs_version = shift;
#  warn "cs_name=", Dumper($cs_name), "\n";
#  warn "cs_version=", Dumper($cs_version), "\n";

  throw('Coord_system name argument is required') if(!$cs_name);

  my $slice_adaptor = $self->adaptor();
 # warn "slice_adaptor=", Dumper($slice_adaptor), "\n";
  if(!$slice_adaptor) {
  ...
  my $db = $slice_adaptor->db();#warn "db=", Dumper($db), "\n";
...
#    warn "asma=", Dumper($asma), "\n";
    my $asm_mapper = $asma->fetch_by_CoordSystems($slice_cs, $cs);
#    warn "asm_mapper=", Dumper($asm_mapper), "\n";
    ...
    foreach my $coord (@coords) {#warn "coord=", Dumper($coord), "\n";
